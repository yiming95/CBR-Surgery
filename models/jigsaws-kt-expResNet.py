...
parser = argparse.ArgumentParser(description='JIGSAWS Knot tying')
parser.add_argument('--validation_option', type=str, default='LOSO1', help='Cross Validation Options')
parser.add_argument('--epochs', type=int, default=300)
args = parser.parse_args()

print("*** JIGSAWS Knot tying ***  " + (args.validation_option) )

(x_train, y_train), (x_validation, y_validation), (x_test, y_test), (x_full_train, y_full_train, x_train_original, x_test_original, KT_Full_Train) = load_data(args.validation_option)
...
resnet_kt_train_feature_vectors = feature_extractor_model.predict(x_train)
resnet_kt_validation_feature_vectors = feature_extractor_model.predict(x_validation)
resnet_kt_test_feature_vectors = feature_extractor_model.predict(x_test) 
...
file_path = './results/jigsaws/' + "resnet_kt_" + str(args.validation_option) + '_results.csv'
df.to_csv(file_path, index=False)

np.save('./results/jigsaws/' + "./resnet_kt_" + str(args.validation_option) + "_train_feature_vectors.npy", resnet_kt_train_feature_vectors)
np.save('./results/jigsaws/' + "./resnet_kt_" + str(args.validation_option) + "_validation_feature_vectors.npy", resnet_kt_validation_feature_vectors)
np.save('./results/jigsaws/' + "./resnet_kt_" + str(args.validation_option) + "_test_feature_vectors.npy", resnet_kt_test_feature_vectors)
