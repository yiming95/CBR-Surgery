...
parser = argparse.ArgumentParser(description='ROSMA Pea on a Peg ResNet')
parser.add_argument('--validation_option', type=str, default='LOSO1', help='Cross Validation Options')
parser.add_argument('--epochs', type=int, default=500)
args = parser.parse_args()

print("*** ROSMA Pea on a Peg ResNet ***  " + (args.validation_option) )
...
resnet_pp_train_feature_vectors = feature_extractor_model.predict(x_train)
resnet_pp_validation_feature_vectors = feature_extractor_model.predict(x_validation)
resnet_pp_test_feature_vectors = feature_extractor_model.predict(x_test) 
...
file_path = './results/rosma/' + "resnet_pp_" + str(args.validation_option) + '_results.csv'
df.to_csv(file_path, index=False)

np.save('./results/rosma/' + "./resnet_pp_" + str(args.validation_option) + "_train_feature_vectors.npy", resnet_pp_train_feature_vectors)

np.save('./results/rosma/' + "./resnet_pp_" + str(args.validation_option) + "_validation_feature_vectors.npy", resnet_pp_validation_feature_vectors)

np.save('./results/rosma/' + "./resnet_pp_" + str(args.validation_option) + "_test_feature_vectors.npy", resnet_pp_test_feature_vectors)
