...
parser = argparse.ArgumentParser(description='ROSMA Pea on a Peg FFT')
parser.add_argument('--validation_option', type=str, default='LOSO1', help='Cross Validation Options')
args = parser.parse_args()

print("*** ROSMA Pea on a Peg FFT ***  " + (args.validation_option) )
...
file_path = './results/rosma/' + "fft_pp_" + str(args.validation_option) + '_results.csv'
df.to_csv(file_path, index=False)

np.save('./results/rosma/' + "./fft_pp_" + str(args.validation_option) + "_train_feature_vectors.npy", fft_train_feature_vectors)
np.save('./results/rosma/' + "./fft_pp_" + str(args.validation_option) + "_validation_feature_vectors.npy", fft_validation_feature_vectors)

np.save('./results/rosma/' + "./fft_pp_" + str(args.validation_option) + "_test_feature_vectors.npy", fft_test_feature_vectors)
