...
parser = argparse.ArgumentParser(description='ROSMA Pea on a peg ROCKET')
parser.add_argument('--validation_option', type=str, default='LOSO1', help='Cross Validation Options')
args = parser.parse_args()

print("*** ROSMA Pea on a peg ROCKET ***  " + (args.validation_option) )
...
# # For logits (probability estimates), you need a calibrated version of RidgeClassifierCV
# calibrated_clf = CalibratedClassifierCV(base_estimator=RidgeClassifierCV(), cv='prefit')
# calibrated_clf.fit(rocket_pipeline[:-1].transform(x_train), y_train)
...
# file_name = f"rocket_pp_multimodality_{validation_option}.csv"
...
####
file_path = './results/rosma/' + "rocket_pp_" + str(args.validation_option) + '_results.csv'
df.to_csv(file_path, index=False)
...
####
np.save('./results/rosma/' + "./rocket_pp_" + str(args.validation_option) + "_train_feature_vectors.npy", train_transformed)
np.save('./results/rosma/' + "./rocket_pp_" + str(args.validation_option) + "_validation_feature_vectors.npy", validation_transformed)
np.save('./results/rosma/' + "./rocket_pp_" + str(args.validation_option) + "_test_feature_vectors.npy", test_transformed)
