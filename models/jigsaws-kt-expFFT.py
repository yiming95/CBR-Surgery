...
parser = argparse.ArgumentParser(description='JIGSAWS Knot tying FFT')
parser.add_argument('--validation_option', type=str, default='LOSO1', help='Cross Validation Options')
args = parser.parse_args()

print("*** JIGSAWS Knot tying FFT ***  " + (args.validation_option) )
...
print("Train original shape:  ", x_train_original.shape)
print("Test original shape:  ", x_test_original.shape)
...
file_path = './results/jigsaws/' + "fft_kt_" + str(args.validation_option) + '_results.csv'
df.to_csv(file_path, index=False)

np.save('./results/jigsaws/' + "./fft_kt_" + str(args.validation_option) + "_train_feature_vectors.npy", fft_train_feature_vectors)
np.save('./results/jigsaws/' + "./fft_kt_" + str(args.validation_option) + "_validation_feature_vectors.npy", fft_validation_feature_vectors)
np.save('./results/jigsaws/' + "./fft_kt_" + str(args.validation_option) + "_test_feature_vectors.npy", fft_test_feature_vectors)
